...
namespace mach
{
    namespace tool
    {
        GnssParse::GnssParse(const ParseFileType &type):BaseParse(type)
        {

        }
        GnssParse::~GnssParse()
        {
            std::cout << "GnssParse Release " << std::endl;
        }
        bool GnssParse::Open(const std::string &file)
        {
            if(m_emType_!=ParseFileType::GNSS_PARAMS)
            {
                return false;
            }
            m_strFileUrl = file;
        ...
        bool GnssParse::ParseData()
        {
            ...
            if(stream.is_open())
            {
                try{
                    ...
                    if(l_bOk)
                    {
                        if(m_emType_ == ParseFileType::GNSS_PARAMS && m_data_.type_ == m_emType_)
                        {
                            if (root.isMember("transform"))
                            {
                                ...
                                m_data_.gnss_data_.transform_.translation_ = Translation(translation_x,translation_y,translation_z);
                                m_data_.gnss_data_.transform_.rotation_ = Rotation(rotation_x,rotation_y,rotation_z,rotation_w);
                                m_data_.gnss_data_.euler_degree_ = Point3D(euler_degree_RotX,euler_degree_RotY,euler_degree_RotZ);
                                m_data_.gnss_data_.calib_status_ = root["calib_status"].asUInt();
                                m_data_.gnss_data_.information_ = root["information"].asString();
                                m_data_.gnss_data_.calib_time_ = root["calib_time"].asString();
                                stream.close();
                                return true;
                            }
                        }
