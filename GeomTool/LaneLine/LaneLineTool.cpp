#include "LaneLineTool.h"
#include "LaneLine/gen.h"
namespace mach
{
    namespace tool
    {
        LaneLineTool::LaneLineTool(const std::string & base_path)
        {
            m_strBasePath_ = base_path;
        }
        LaneLineTool::~LaneLineTool()
        {
        }
        void LaneLineTool::GenVoxelMapping(const std::string &geom_path, const std::string &voxel_path)
        {
            m_vecKeys.clear();
            try
            {
                xt::xarray<int64_t> geom;
                if(GetType(geom_path)=="'<i8'")
                {
                    geom = xt::load_npy<int64_t>(geom_path);
                    // std::cout<<geom_path<<" i8 "<<std::endl;
                }
                if(GetType(geom_path)=="'<i4'")
                {
                    geom = xt::load_npy<int32_t>(geom_path);
                    // std::cout<<geom_path<<" i4 "<<std::endl;
                }
                        ...
                        for (size_t d = 0; d < D; ++d) {
                            for (size_t h = 0; h < H; ++h) {
                                for (size_t w = 0; w < W; ++w) {
                                    int64_t x = geom(b, n, d, h, w, 0);
                                    int64_t y = geom(b, n, d, h, w, 1);
                                    int64_t z = geom(b, n, d, h, w, 2);
                ...
                // stream.close();
                ParseGeomS8(voxel_mapping,m_strBasePath_ + "/output/geom_xyz_valid.npy");         
                GenBin();
            }
            catch(std::runtime_error& e)
            {
                std::cout<<"UpdateNpy runtime_error "<<geom_path<<": " << e.what() << std::endl;
            }
            catch (std::exception)
            {
                std::cout<<"UpdateNpy exception "<<geom_path<<std::endl;
            }
            catch(const std::exception& e)
            {
                std::cout<<"UpdateNpy exception "<<e.what()<<geom_path<<std::endl;
            }
        }
        void LaneLineTool::ParseGeomS8(VoxelMapping voxel_mapping, const std::string &output_path)
        {
            int valid_points2 = 0;
            ...
            size_t nidx = 0;
            for(const VoxelKey &key : m_vecKeys)
            {
                int b_voxel, z, y, x;
        ...
        std::string LaneLineTool::GetType(const std::string &filename)
        {
        ...
        void LaneLineTool::GenBin()
        {
            test_lvt_lane_line(m_strBasePath_);
        }
    }
}
