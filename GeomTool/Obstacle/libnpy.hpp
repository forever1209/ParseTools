...
const std::unordered_map<std::type_index, dtype_t> dtype_map = {
        {std::type_index(typeid(float)),
         {host_endian_char, 'f', sizeof(float)}},
        {std::type_index(typeid(double)),
         {host_endian_char, 'f', sizeof(double)}},
        ...
        {std::type_index(typeid(short)),
         {host_endian_char, 'i', sizeof(short)}},
...
template <typename Scalar>
inline size_t LoadArrayFromNumpy(const std::string& filename, Scalar* data) {
    ...
    size_t size = static_cast<size_t>(comp_size(header.shape));

    // read the data
    if(data)
        stream.read(reinterpret_cast<char*>(data), sizeof(Scalar) * size);

    return size;
}

inline std::vector<ndarray_len_t> LoadArrayShapeFromNumpy(const std::string& filename) {
    std::ifstream stream(filename, std::ios::binary);
    if (!stream) {
        std::cerr << "Error: " << std::strerror(errno) << std::endl;
        throw std::runtime_error("io error: failed to open a file.");
